...
function *getHandler() {
    ...
    teacherFeedback = yield this.knex('teacher_feedback')
        .leftJoin('local_people', 'teacher_feedback.author_id', 'local_people.ID')
        .select('teacher_feedback.*', this.knex.raw(`(local_people."FirstName" || ' ' || local_people."LastName") AS author_name`))
        .where(query);
