...
folder are mapped to the application's root namespace, 'SparkClassroomStudent'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SparkClassroomStudent"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SparkClassroomStudent' in the same way
as `"SparkClassroomStudent/sass/src"`.
