...
class LookupTable {
    ...
    constructor(options) {
        return (async () => {
            ...
            if (this.autoBust) {
                ...
                this.natsClient.subscribe(subject, async (msg, reply, subject) => {
                        ...
                        if (self.onCacheBust) {
                            try {
                                if (self.onCacheBust.constructor.name === 'GeneratorFunction') {
                                    await self.onCacheBust.apply(self, [event]);
                                } else {
    ...
    async populate(pgp) {
        var {entity, tableName, idColumn, codeColumn, additionalColumns, customFunction, customGenerator, schema, cache} = this,
            ...
            columnsQuoted = columns
                .map(column => `"${column}"`)
                .join(','),
            results = await (pgp || this.pgp).any(`
        ...
        if (customFunction) {
            if (customFunction.constructor.name === 'AsyncFunction') {
                await customFunction.apply(this, [results, columns]);
            } else {
    ...
    async idToCode(id) {
        ...
        if (cachedCode) {
            ...
            let value = await this.pgp.oneOrNone(
