...
Ext.define('Ext.saki.grid.MultiSearch', {
    ...
    getFields: function() {
        ...
        Ext.Array.each(gridCols, function(item, i) {
            ...
            } else // otherwise column shouldn't be filtered
            {
    ...
    doFieldChange: function(field) {
        var me = this,
            ...
            filter;
        filter = parse ? me.parseUserValue(value) : {
            value: value
        };
    ...
    updateClearIcon: function(field) {
        var me = this,
            ...
            body = field.bodyEl ? field.bodyEl.down('input') : null,
            //.up('div')
        ...
        if (body) {
            ...
            if (false !== field.clearIcon) {
                if (!field.clearIcon) {
                    ...
                    field.clearIcon.on('click', Ext.bind(me.clearField, me, [
                        field
                    ]));
                    body.applyStyles({
                        position: 'relative'
    ...
    markFiltered: function(field) {
        var me = this,
            value = field.getValue ? field.getValue() : null,
            colEl = me.headerCt.getGridColumns()[me.items.indexOf(field)];
        //.getEl()
        if (!colEl) {
            return;
        }
